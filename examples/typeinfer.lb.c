...
static inline char* _lb_u8(uint8_t x)   { snprintf(_lb_buf,sizeof(_lb_buf),"%" PRIu8,x); return _lb_buf; }
static inline char* _lb_u32(uint32_t x) { snprintf(_lb_buf,sizeof(_lb_buf),"%" PRIu32,x); return _lb_buf; }
static inline char* _lb_u64(uint64_t x) { snprintf(_lb_buf,sizeof(_lb_buf),"%" PRIu64,x); return _lb_buf; }
#define TO_STR(x) _Generic((x),char*:_lb_s,const char*:_lb_cs,__m256:_lb_m,__m256i:_lb_mi,float:_lb_f,double:_lb_d,int:_lb_i,long:_lb_l,short:_lb_u,uint8_t:_lb_u8,uint32_t:_lb_u32,uint64_t:_lb_u64,bool:_lb_b,char:_lb_c,default:_lb_i)(x)
...
#line 2 "/home/mebecool/Coding/lbreal/luaBase-Language/examples/typeinfer.lb"
    return (x+y);
}

int main(int argc, char** argv) {
    #line 4 "/home/mebecool/Coding/lbreal/luaBase-Language/examples/typeinfer.lb"
printf("%s",TO_STR(x(1, 2))); printf("\n");
    #line 5 "/home/mebecool/Coding/lbreal/luaBase-Language/examples/typeinfer.lb"
printf("%s",TO_STR(x(1.2, 2.2))); printf("\n");
    #line 6 "/home/mebecool/Coding/lbreal/luaBase-Language/examples/typeinfer.lb"
printf("%s",TO_STR(x(1, 2.2))); printf("\n");
    return 0;
}